...
def get_robot_arm_urdf_path():
    return str(
        (Path(get_package_share_directory("robot_arm_description")) / "urdf"
         / "right_left_arm.urdf")
    )
...
class Kinematics:
    ...
    def createSolver(self):
        ...
        self.translational_cost = casadi.sumsqr(
            self.translational_error(self.var_q, self.param_tf_r, self.param_tf_l))
        self.rotation_cost = casadi.sumsqr(self.rotational_error(
            self.var_q, self.param_tf_r, self.param_tf_l))
        ...
        self.opti.minimize(80.0 * self.translational_cost + 20 * self.rotation_cost +
                           0.1 * self.regularization_cost + 1.0 * self.smooth_cost)
    ...
    def debug_forward_kinematics(self, q=None):
        ...
        try:
            ...
            #     print(f"{i}: {name:<24} : {oMi.translation[0]:.2f} {oMi.translation[1]:.2f}
            # {oMi.translation[2]:.2f}")
            # 打印每个连杆的位置
            # print("各连杆位置:")
            # for i, frame in enumerate(self.model.frames):
            #     frame_name = frame.name
            #     frame_placement = self.data.oMf[i]
            #     print(f"{i}: {frame_name:<24} : {frame_placement.translation[0]:.2f}
            # {frame_placement.translation[1]:.2f} {frame_placement.translation[2]:.2f}")
...
class JointStateSubscriber(Node):
    ...
    def listener_callback(self, msg):
        # 定义您关心的关节顺序
        # desired_joints = ['rjoint1', 'rjoint2', 'rjoint3', 'rjoint4', 'rjoint5', 'rjoint6',
        #     'rjoint7'
        ...
        [self.current_pos[0], self.current_pos[1]], [self.current_rpy[0],
                                                     self.current_rpy[1]] = (
                self.ik.debug_forward_kinematics(self.sorted_positions))
    ...
    def twist_callback(self, msg):
        ...
        # 获取旋转矩阵
        R_right = self.rpy_to_rotation_matrix(
            msg.right_arm_twist.angular.x, msg.right_arm_twist.angular.y,
            msg.right_arm_twist.angular.z)
        R_left = self.rpy_to_rotation_matrix(
            msg.left_arm_twist.angular.x, msg.left_arm_twist.angular.y,
            msg.left_arm_twist.angular.z)

        # R_right = self.rpy_to_rotation_matrix(0, 0, 0)
        # R_left = self.rpy_to_rotation_matrix(0, 0, 0)
        # 创建3x4的变换矩阵（旋转 + 平移）
        translation_right = np.array([msg.right_arm_twist.linear.x, msg.right_arm_twist.linear.y,
                                     msg.right_arm_twist.linear.z])  # + self.current_pos[0] # 平移向量
        tf_3x4_right = np.hstack((R_right, translation_right.reshape(3, 1)))
        translation_left = np.array([msg.left_arm_twist.linear.x, msg.left_arm_twist.linear.y,
                                    msg.left_arm_twist.linear.z])  # + self.current_pos[1] # 平移向量
