...
typedef struct
{
  ...
  /** Axis I/O tail of the driver's ec_app_read_reg_t.  Without it this struct
   *  is 464 bytes, and init_shm_desire() would truncate the live
   *  Ethercat_axis_desire segment below the 480 bytes that the IGH driver and
   *  the standalone hardware console map, making them fail with "mmap length
   *  is greater than file size" until the driver grew it again. */
  uint8_t ec_axis_io[16];
} ec_app_desire_reg_t;

/** The IGH driver truncates these two POSIX segments to sizeof(ec_app_read_reg_t)
 *  (480) and sizeof(ec_app_write_reg_t) (648).  Keep the sizes asserted so this
 *  side can never silently shrink a live shared-memory segment again. */
static_assert(
  sizeof(ec_app_desire_reg_t) == 480,
  "Ethercat_axis_desire ABI must match the IGH driver");
static_assert(sizeof(ec_app_real_reg_t) == 648, "Ethercat_axis_real ABI must match the IGH driver");

void get_joint_real(
  ec_app_axis_read_reg_t & real_reg, MotorState & motor_state,
  uint8_t motor_class, uint8_t motor_CW)
{
  motor_state.error_code = static_cast<double>(real_reg.axis_error_code);
  switch (motor_class) {
    case ZERO:
      if (motor_CW == 1) {
        motor_state.position = ((double)real_reg.axis_position) *
          -1.1984224905356572107172559292906e-5;
        motor_state.velocity = ((double)real_reg.axis_velocity) *
          -1.1984224905356572107172559292906e-5;
...
void set_joint_desire(
  ec_app_axis_write_reg_t & desire_reg, MotorState & motor_desire,
  uint8_t motor_class, uint8_t motor_CW, uint8_t motor_mode_)
{
  ...
  switch (motor_class) {
    case ZERO:
      ...
      } else if (motor_mode_ == RunMode::VELOCITY) {
        desire_reg.ec_mode = motor_mode::CSV;
