...
def generate_launch_description():
    config_path = (
        Path(get_package_share_directory("joint_hardware")) / "config" / "lift_hardware.yaml"
    )
    ...
    # `zero_offset_file` defaults to a $HOME-based host path, so it cannot go
    # through cfg(): cfg() stringifies whatever it is given and would turn the
    # substitution into a literal repr.  A host-specific absolute path declared in
    # lift_hardware.yaml still wins, which keeps the yaml honest.  Previously the
    # yaml declared /tmp/joint_hardware_lift_zero_offset.cfg while this launch
    # silently used $HOME/.local/state/joint_controller/lift_zero_offset.cfg, so
    # editing the yaml looked like it had no effect and an operator could not tell
    # which file actually held the host zero.
    zero_offset_file_default: object = PathJoinSubstitution(
        [
    ...
    _configured_zero_offset_file = str(lift.get("zero_offset_file") or "").strip()
    if _configured_zero_offset_file:
        zero_offset_file_default = _configured_zero_offset_file
    ...
    argument_specs = [
        ...
        (
            "master_index",
            cfg("ethercat_master_index", 2),
            "EtherLab master index; 0/1 are reserved for the arms",
        ),
        ("cycle_ms", cfg("ethercat_cycle_ms", 10), "EtherCAT cycle in milliseconds"),
        (
            "expected_working_counter",
            cfg("expected_working_counter", 1),
            "minimum complete PDO working counter",
        ),
        ...
        (
            "ethercat_interface",
            cfg("ethercat_interface", "enp5s0"),
            "EtherCAT interface name used by the master config",
        ),
        ...
        (
            "command_units_per_rev",
            cfg("command_units_per_rev", 10000),
            "validated 6092/P00.08 command units per rev",
        ),
        (
            "encoder_counts_per_rev",
            cfg("encoder_counts_per_rev", 131072),
            "validated 608F encoder counts per rev",
        ),
        (
            "lead_mm_per_rev",
        ...
        (
            "brake_control_enabled",
            cfg("brake_control_enabled", False),
            "allow explicit Operation enabled service requests",
        ),
        (
            "limit_switch_enabled",
            cfg("limit_switch_enabled", False),
            "enable optional physical limit input",
        ),
        (
            "limit_switch_positive_bit",
            cfg("limit_switch_positive_bit", -1),
            "60FD bit for the positive travel limit",
        ),
        (
            "limit_switch_negative_bit",
            cfg("limit_switch_negative_bit", -1),
            "60FD bit for the negative travel limit",
        ),
        (
            "limit_switch_active_high",
            cfg("limit_switch_active_high", True),
            "whether an asserted limit is a high bit",
        ),
        (
            "brake_release_wait_ms",
            cfg("brake_release_wait_ms", -1),
            "confirmed P04.38 release delay",
        ),
        ...
        (
            "brake_p05_06_mode",
            cfg("brake_p05_06_mode", "drive_default"),
            "expected P05.06 disable mode",
        ),
        (
            "brake_p05_10_mode",
            cfg("brake_p05_10_mode", "drive_default"),
            "expected P05.10 alarm-stop mode",
        ),
