...
#include <robot_control_msg/srv/cartesian_path_increment_control.hpp>
...
class CartesianPathIncrementControlSrv : public rclcpp::Node
{
public:
  CartesianPathIncrementControlSrv() : Node("cartesian_path_increment_control_srv")
  {
    ...
    motion_status_sub_ = this->create_subscription<robot_control_msg::msg::ArmMotionStatus>(
      ...
      std::bind(&CartesianPathIncrementControlSrv::motionStatusCallback, this, std::placeholders::_1),
      topic_sub_options);

    tcp_pose_sub_ = this->create_subscription<robot_control_msg::msg::EndEffectorPose>(
      "/arm_tcp_pose", 10,
      std::bind(&CartesianPathIncrementControlSrv::tcpPoseCallback, this, std::placeholders::_1),
      topic_sub_options);
    ...
    execution_status_sub_ =
      this->create_subscription<robot_control_msg::msg::CartesianExecutionStatus>(
        ...
        std::bind(
          &CartesianPathIncrementControlSrv::executionStatusCallback, this, std::placeholders::_1),
        topic_sub_options);

    path_cmd_pub_ = this->create_publisher<robot_control_msg::msg::Robotarmmovelpath>(
      "/arm_cartesian_path_cmd", 10);

    increment_control_srv_ = this->create_service<robot_control_msg::srv::CartesianPathIncrementControl>(
      "cartesian_path_increment_control",
      std::bind(
        &CartesianPathIncrementControlSrv::handleIncrementPathRequest, this,
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "Cartesian Path Increment Control Service is ready on cartesian_path_increment_control");
  }
...
private:
  ...
  bool validateIncrementRequest(
    const robot_control_msg::srv::CartesianPathIncrementControl::Request& request,
  ...
  bool accumulateArmWaypoints(
    const std::vector<geometry_msgs::msg::Pose>& increments, const PoseState& start_pose,
    std::vector<geometry_msgs::msg::Pose>& absolute_waypoints, PoseState& final_pose,
    double& path_length, bool& has_motion, std::string& error, const std::string& arm_name) const
  {
    PoseState current_pose = start_pose;
    absolute_waypoints.clear();
    absolute_waypoints.reserve(increments.size());
    path_length = 0.0;
    has_motion = false;

    for (size_t i = 0; i < increments.size(); ++i) {
      PoseState increment_pose;
      if (!poseFromMsg(
            increments[i], increment_pose, error,
            arm_name + "_waypoints[" + std::to_string(i) + "]")) {
        return false;
      }

      const PoseState previous_pose = current_pose;
      current_pose.position += increment_pose.position;
      current_pose.orientation =
        normalizeQuaternion(current_pose.orientation * increment_pose.orientation);

      geometry_msgs::msg::Pose absolute_pose_msg;
      absolute_pose_msg.position.x = current_pose.position.x();
      absolute_pose_msg.position.y = current_pose.position.y();
      absolute_pose_msg.position.z = current_pose.position.z();
      absolute_pose_msg.orientation.x = current_pose.orientation.x();
      absolute_pose_msg.orientation.y = current_pose.orientation.y();
      absolute_pose_msg.orientation.z = current_pose.orientation.z();
      absolute_pose_msg.orientation.w = current_pose.orientation.w();
      absolute_waypoints.push_back(absolute_pose_msg);
  ...
  void handleIncrementPathRequest(
    const std::shared_ptr<robot_control_msg::srv::CartesianPathIncrementControl::Request> request,
    std::shared_ptr<robot_control_msg::srv::CartesianPathIncrementControl::Response> response)
  {
    RCLCPP_INFO(this->get_logger(), "Received cartesian path increment control request");
    ...
    if (!validateIncrementRequest(*request, error)) {
      response->success = false;
      response->message = error;
      RCLCPP_ERROR(this->get_logger(), "Invalid increment path request: %s", error.c_str());
      return;
    }
    ...
    std::vector<geometry_msgs::msg::Pose> absolute_left_waypoints;
    std::vector<geometry_msgs::msg::Pose> absolute_right_waypoints;
    ...
    if (!request->left_waypoints.empty()) {
      if (!accumulateArmWaypoints(
            request->left_waypoints, current_left, absolute_left_waypoints, target_left,
            left_path_length, left_has_motion, error, "left")) {
        response->success = false;
        // 在错误消息中包含当前状态信息
        std::string detailed_error = error;
        if (has_execution_status_) {
    ...
    if (!request->right_waypoints.empty()) {
      if (!accumulateArmWaypoints(
            request->right_waypoints, current_right, absolute_right_waypoints, target_right,
            right_path_length, right_has_motion, error, "right")) {
        response->success = false;
        // 在错误消息中包含当前状态信息
        std::string detailed_error = error;
        if (has_execution_status_) {
    ...
    cmd_msg.use_left = !absolute_left_waypoints.empty();
    cmd_msg.use_right = !absolute_right_waypoints.empty();
    cmd_msg.left_waypoints = absolute_left_waypoints;
    cmd_msg.left_blend_radii = request->left_blend_radii;
    cmd_msg.right_waypoints = absolute_right_waypoints;
    ...
    response->message = "Cartesian path increment control completed successfully";
    RCLCPP_INFO(
      this->get_logger(),
      "Cartesian path increment motion completed successfully (left_length=%.4f m, right_length=%.4f m, left_motion=%d, right_motion=%d)",
  ...
  rclcpp::Service<robot_control_msg::srv::CartesianPathIncrementControl>::SharedPtr
    increment_control_srv_;
...
int main(int argc, char* argv[])
{
  ...
  auto node = std::make_shared<teleop_control::CartesianPathIncrementControlSrv>();
