...
class CartesianIncrementControlSrv : public rclcpp::Node
{
public:
    CartesianIncrementControlSrv() : Node("cartesian_increment_control_srv")
    {
        /* ------------------- 参数 ------------------- */
        ...
        joint_state_sub_ = this->create_subscription<sensor_msgs::msg::JointState>(
            ...
            std::bind(&CartesianIncrementControlSrv::jointStateCallback, this, std::placeholders::_1),
            sub_options);
        ...
        motion_status_sub_ = this->create_subscription<robot_control_msg::msg::ArmMotionStatus>(
            ...
            std::bind(&CartesianIncrementControlSrv::motionStatusCallback, this, std::placeholders::_1),
            motion_sub_options);
        ...
        execution_status_sub_ = this->create_subscription<robot_control_msg::msg::CartesianExecutionStatus>(
            ...
            std::bind(&CartesianIncrementControlSrv::executionStatusCallback, this, std::placeholders::_1),
            motion_sub_options);

        // 创建服务服务器，绑定到 service_cb_group_
        // 使用 rmw_qos_profile_services_default 作为 QoS 参数
        increment_control_srv_ = this->create_service<robot_control_msg::srv::CartesianIncrementControl>(
            "cartesian_increment_control",
            std::bind(&CartesianIncrementControlSrv::handleIncrementControlRequest, this, 
                     std::placeholders::_1, std::placeholders::_2),
            rmw_qos_profile_services_default,
            service_cb_group_);

        RCLCPP_INFO(this->get_logger(), "Cartesian Increment Control Service is ready.");
    }
...
private:
    ...
    /* === 服务回调 === */
    void handleIncrementControlRequest(
        const std::shared_ptr<robot_control_msg::srv::CartesianIncrementControl::Request> request,
        std::shared_ptr<robot_control_msg::srv::CartesianIncrementControl::Response> response)
    {
        RCLCPP_INFO(this->get_logger(), "Received cartesian increment control request");
        ...
        robot_control_msg::safety::RobotCommandGuard command_guard(
            "legacy_cartesian_increment_control");
        ...
        // 1. 获取当前TCP位姿（考虑TCP偏移）
        Eigen::Vector3d current_pos_L, current_pos_R;
        Eigen::Matrix3d current_rot_L, current_rot_R;
        ...
        auto M_ee_L = data_.oMf[ee_id_left_];
        auto M_tcp_L = M_ee_L * tcp_transform_left_;
        current_pos_L = M_tcp_L.translation();
        current_rot_L = M_tcp_L.rotation();

        auto M_ee_R = data_.oMf[ee_id_right_];
        auto M_tcp_R = M_ee_R * tcp_transform_right_;
        current_pos_R = M_tcp_R.translation();
        current_rot_R = M_tcp_R.rotation();

        // 2. 计算目标TCP位姿
        Eigen::Vector3d target_pos_L = current_pos_L + 
            Eigen::Vector3d(request->lx, request->ly, request->lz);
        
        Eigen::Vector3d target_pos_R = current_pos_R + 
            Eigen::Vector3d(request->rx, request->ry, request->rz);

        // 3. 计算目标姿态（姿态增量叠加到当前姿态）
        Eigen::Matrix3d target_rot_L;
        if (std::abs(request->lqx) + std::abs(request->lqy) + std::abs(request->lqz) + std::abs(request->lqw) > 1e-6) {
            Eigen::Quaterniond q_inc(request->lqw, request->lqx, request->lqy, request->lqz);
            Eigen::Matrix3d R_inc = q_inc.toRotationMatrix();
            target_rot_L = current_rot_L * R_inc;
        } else if (std::abs(request->lroll) + std::abs(request->lpitch) + std::abs(request->lyaw) > 1e-6) {
            Eigen::Matrix3d R_inc = rpyToRot(request->lroll, request->lpitch, request->lyaw);
            target_rot_L = current_rot_L * R_inc;
        } else {
            target_rot_L = current_rot_L;
        }

        Eigen::Matrix3d target_rot_R;
        if (std::abs(request->rqx) + std::abs(request->rqy) + std::abs(request->rqz) + std::abs(request->rqw) > 1e-6) {
            Eigen::Quaterniond q_inc(request->rqw, request->rqx, request->rqy, request->rqz);
            Eigen::Matrix3d R_inc = q_inc.toRotationMatrix();
            target_rot_R = current_rot_R * R_inc;
        } else if (std::abs(request->rroll) + std::abs(request->rpitch) + std::abs(request->ryaw) > 1e-6) {
            Eigen::Matrix3d R_inc = rpyToRot(request->rroll, request->rpitch, request->ryaw);
            target_rot_R = current_rot_R * R_inc;
        } else {
            target_rot_R = current_rot_R;
        }

        // 5. 发布到现有的笛卡尔控制话题
        auto cmd_msg = robot_control_msg::msg::Robotarmmovel();
        
        cmd_msg.lx = target_pos_L.x();
        cmd_msg.ly = target_pos_L.y();
        cmd_msg.lz = target_pos_L.z();
        
        Eigen::Quaterniond q_target_L(target_rot_L);
        cmd_msg.lqx = q_target_L.x();
        cmd_msg.lqy = q_target_L.y();
        cmd_msg.lqz = q_target_L.z();
        cmd_msg.lqw = q_target_L.w();
        
        cmd_msg.rx = target_pos_R.x();
        cmd_msg.ry = target_pos_R.y();
        cmd_msg.rz = target_pos_R.z();
        
        Eigen::Quaterniond q_target_R(target_rot_R);
        cmd_msg.rqx = q_target_R.x();
        cmd_msg.rqy = q_target_R.y();
        cmd_msg.rqz = q_target_R.z();
        cmd_msg.rqw = q_target_R.w();
        ...
        if (!waitForCommandAcceptance(
            ...
            response->message = detailed_error;
            RCLCPP_ERROR(this->get_logger(), "Motion failed during increment control: %s", detailed_error.c_str());
            return;
        }

        // 6. 等待运动完成（基于TCP位姿检查）
        if (waitForArmToReachGoal(target_pos_L, target_rot_L, target_pos_R, target_rot_R)) {
            response->success = true;
            response->message = "Cartesian increment control completed successfully";
    ...
    rclcpp::Service<robot_control_msg::srv::CartesianIncrementControl>::SharedPtr increment_control_srv_;
...
int main(int argc, char * argv[])
{
    ...
    auto node = std::make_shared<teleop_control::CartesianIncrementControlSrv>();

    // 使用多线程执行器，设置2个线程（可根据需要调整）
